...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resize_attention_image",
        type=str,
        default=True,
        help="Wheter to resize image or not",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_images", default=None, type=int, help="Number of images to run in total."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_per_class", default=False, type=bool, help="Use custom embedding map"
    )
    parser.add_argument(
...
model = vits.__dict__[args.arch](
    patch_size=args.patch_size, num_classes=0, in_chans=num_in_chans_pretrained
)
...
if not args.use_mean_patch_embedding and not args.use_custom_embedding_map:
    if len(selected_channels) != num_in_chans_pretrained:
        print(
            f"Error: Number of channels in the dataset ({len(selected_channels)}) and pretrained weights ({num_in_chans_pretrained}) are different"
        )
        print(
...
classes = utils.fetch_all_folder_names_of_folder_depth(
    dataset_total, folder_depth=args.folder_depth_for_labels
)
...
if args.scDINO_full_pipeline:
    ...
    run_directory = os.path.join(
        args.output_dir,
        args.name_of_run,
        f"{args.full_ViT_name}_analyses/attention_images/epoch{epoch_num}",
    )
else:
    ...
    run_directory = os.path.join(
        args.output_dir,
        args.name_of_run,
        f"attention_images/channel{channel_names}_model_{checkpoint}",
    )
try:
...
for class_name in classes:
    ...
    class_indices = [
        i
        for i, x in enumerate(dataset_total.imgs)
        if class_name == x[0].split("/")[-args.folder_depth_for_labels - 2]
    ]
    print(
        f"Computing attention images for class {class_name} with {len(class_indices)} images"
    )
    print(class_indices)
    if args.scDINO_full_pipeline:  # subsample from val_indices
        ...
        val_indices_intersect = list(
            set(val_indices) & set(list(map(int, class_indices)))
        )
    ...
    for cell in random_indices:
        ...
        for channel in selected_channels:
            ...
            torchvision.utils.save_image(
                torchvision.utils.make_grid(og_img, normalize=True, scale_each=True),
                os.path.join(
                    f"{random_cell_dir}/original_img_channel"
                    + str(channel_name)
                    + ".png"
                ),
            )
...
with open(os.path.join(run_directory, "run_log.txt"), "w") as f:
    f.write(
        f"successfully computed attention visualisation with following parameters and random seed {args.seed}: \n"
    )
    f.write("parameters: \n")
