...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        help="""path to mean and std file in json format""",
    )
    parser.add_argument(
        "--upscale_factor",
        default=0,
        type=float,
        help="""upscale factor to upsample images""",
    )
    parser.add_argument(
        "--center_crop",
        default=0,
        type=int,
        help="""center crop factor to crop images""",
    )
    ...
    parser.add_argument(
        ...
        default=0.8,
        type=float,
        help="fraction of training dataset to use for training",
    )
    parser.add_argument(
        "--dino_vit_name", default="vit_small_p16", type=str, help="name of ViTs"
    )
    parser.add_argument(
        "--channel_dict",
        default=None,
...
def train_dino(args, save_dir):
    ...
    def load_mean_std_per_channel(norm_per_channel_file):
        with open(args.norm_per_channel_file) as f:
            ...
            norm_per_channel = [
                norm_per_channel_json["mean"],
                norm_per_channel_json["std"],
            ]
        return norm_per_channel

    def create_mean_std_per_channel_for_channel_combi(
        norm_per_channel_file, selected_channels
    ):
        ...
        ), tuple([norm_per_channel[1][std] for std in selected_channels])
        return mean_for_selected_channel, std_for_selected_channel

    selected_channels = list(map(int, args.selected_channels))
    mean_for_selected_channel, std_for_selected_channel = (
        create_mean_std_per_channel_for_channel_combi(
            args.norm_per_channel_file, selected_channels
        )
    )
    ...
    if args.arch in vits.__dict__.keys():
        ...
        teacher = vits.__dict__[args.arch](
            patch_size=args.patch_size, in_chans=len(selected_channels)
        )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.saveckp_freq and epoch % args.saveckp_freq == 0:
            utils.save_on_master(
                save_dict, os.path.join(save_dir, f"checkpoint{str(epoch)}.pth")
            )
        log_stats = {
