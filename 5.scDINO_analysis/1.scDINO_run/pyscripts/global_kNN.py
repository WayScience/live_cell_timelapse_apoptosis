...
for run_index, features in enumerate(snakemake.input["features"]):
    ...
    if snakemake.params["scDINO_full_pipeline"]:
        ...
        neighbors = snakemake.config["downstream_analyses"]["kNN"]["global"][
            "n_neighbors"
        ]
        temperature = snakemake.config["downstream_analyses"]["kNN"]["global"][
            "temperature"
        ]
    else:
    ...
    train_labels = torch.from_numpy(
        label_to_number(class_labels[: int(features.shape[0] * 0.8)])
    )
    ...
    def knn_classifier(
        ...
        for idx in range(0, num_test_images, imgs_per_chunk):
            ...
            top2 = (
                top2 + correct.narrow(1, 0, min(2, k)).sum().item()
