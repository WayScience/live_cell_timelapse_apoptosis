...
umap_embedding = fit_umap(
    features, n_neighbors=15, min_dist=0.1, metric=metric, spread=spread, epochs=epochs
)
...
def make_plot(
    ...
    sns_plot.set_title(
        "CLS Token embedding of "
        + str(len(labels))
        + " cells with a dimensionality of "
...
make_plot(
    ...
    file_name=file_name,
    name="umap",
    description=f"n_neighbors:{n_neighbors}, min_dist={min_dist}, metric={metric}, spread={spread}, epochs={epochs}",
)
...
if snakemake.params["topometry_plots"]:
    ...
    make_plot(
        tg.db_fuzzy_MAP, class_labels, name="db_fuzzy_MAP", save_dir=save_dir_topo
    )
    make_plot(tg.db_diff_MDE, class_labels, name="db_diff_MDE", save_dir=save_dir_topo)
    make_plot(
        tg.db_fuzzy_MDE, class_labels, name="db_fuzzy_MDE", save_dir=save_dir_topo
    )
    ...
    make_plot(
        tg.fb_fuzzy_MAP, class_labels, name="fb_fuzzy_MAP", save_dir=save_dir_topo
    )
    make_plot(tg.fb_diff_MDE, class_labels, name="fb_diff_MDE", save_dir=save_dir_topo)
    make_plot(
        tg.fb_fuzzy_MDE, class_labels, name="fb_fuzzy_MDE", save_dir=save_dir_topo
    )
