...
def apply_alignment(input_image, off_x, off_y, shape):
    ...
    if input_image.ndim == 2:
        ...
        output_shape = (shape[0], shape[1], input_image.shape[2])
        planes = [input_image[:, :, i] for i in range(input_image.shape[2])]
    output_pixels = np.zeros(output_shape, input_image.dtype)
